...
function getEdgePoints(start, end) {
    const startCenterX = start.x;
    const startCenterY = start.y;
    ...
    // Berechnung des richtigen Randabstands für Kreise (Start- und Endzustand)
    const startPadding = (start.type === "start" || start.type === "end") ? start.width / 2 : start.width / 2;
    const endPadding = (end.type === "start" || end.type === "end") ? end.width / 2 : end.width / 2;

    // Startpunkt am Rand des Startzustands
...
canvas.addEventListener("mousedown", (event) => {
    ...
    // Auswahl eines Zustands oder Start/Endzustands
    selectedState = states.find(s => {
        if (s.type === "start" || s.type === "end") {
            // Größere Toleranz für kreisförmige Zustände
            const dx = x - s.x;
            const dy = y - s.y;
            return Math.sqrt(dx * dx + dy * dy) <= s.width / 2 + 5;
        } else {
...
function handleTransitionClick(event) {
    ...
    if (targetState && targetState !== startTransitionState) {
        transitions.push({ start: startTransitionState, end: targetState, label: "Ereignis" });
    }
...
// Buttons für Start- und Endzustand hinzufügen
document.getElementById("controls").innerHTML += `
    <button onclick="addStartState()">🔘 Startzustand</button>
    <button onclick="addEndState()">⭕ Endzustand</button>
`;
