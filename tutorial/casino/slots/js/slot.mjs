...
export function Slot(options) {
  ...
  this.evaluateWin = () => {
    ...
    console.log("bet", this.player.getBet());

    // aufrunden totalwin


    let totalWin = Math.ceil((this.player.getBet() / 3)* winners.reduce((acc, { money }) => acc + money, 0));


    for (const winner of winners) {
      // We have a winner, let's highlight the blocks
      this.visualEffects.highlight(winner.blocks);
      console.info(`Winner: ${winner.type}, line: ${winner.rowIndex}, win: ${totalWin}`);
    }
  ...
  function setCoins(newCoins) {
    ...
    updateDoc(userDocRef, { coins: newCoins })
        .then(() => {
          document.getElementById('coinsshow-text').innerText = newCoins + " Chips";
  ...
  // getbet from player
    this.getBet = () => this.player.getBet();
  ...
  this.subscribeBodyClick = () => {
    console.log('Subscribing to body click event');
  };
